...
ann450k_passQC<- ann450k[row.names(ann450k) %in% row.names(mVals), c("Relation_to_Island","UCSC_RefGene_Group")]
ann450k_passQC["cpg_ID"]<-row.names(ann450k_passQC)


#List of data.frames with methylation values separated by island type

plotALL_byType <-function(bVals, by_islands, outFile, classif){
  pdf(paste0(outFile))
  filteredVals<-list(length(by_islands))
  for(i in by_islands){
    typeName<-i$Relation_to_Island[1]
...
plotALL_byType(bVals, by_islands, outFile, classif=)

outFile<-paste(resultsDirectory,"ALL_densities_byUCSC_RefGene_Group.pdf")
plotALL_byType(bVals, by_refGene, outFile)

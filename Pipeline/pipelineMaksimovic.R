...
ann450kSub <- ann450k[
  match(rownames(mVals), ann450k$Name),
...
# CpG islands
islandHMM <- read.csv(
  paste(dataDirectory, "model-based-cpg-islands-hg19-chr17.txt", sep="/"),
  sep = "\t", 
  stringsAsFactors = FALSE,
  header = FALSE
)
...
dnase <- read.csv(
  paste(dataDirectory,"wgEncodeRegDnaseClusteredV3chr17.bed", sep = "/"),
  sep = "\t",
  stringsAsFactors = FALSE,
  header = FALSE
)
...
cpgData <- GRanges(
  seqnames = Rle(ann450kOrd$chr),
  ranges = IRanges(
    start = ann450kOrd$pos, 
    end = ann450kOrd$pos
  ),
  strand = Rle(
    rep("*",nrow(ann450kOrd))),
